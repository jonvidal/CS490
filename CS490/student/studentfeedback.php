<?php
$quiztitle = $_POST['mygrade'];

$curl_feedback = curl_init("http://afsaccess1.njit.edu/~jkv5/CS490/studentmiddle_quiz.php");
curl_setopt_array($curl_feedback, array(
  CURLOPT_POST =>true,
  CURLOPT_POSTFIELDS =>json_encode(array('username'=>$user, 'quiztitle'=>$quiztitle,'role' => 'studentfeedback')),
  CURLOPT_RETURNTRANSFER =>true,
));
$result_feedback = curl_exec($curl_feedback);
curl_close($curl_feedback);
$results_release = json_decode($result_feedback,true);
//var_dump($results_release);
if($results_release['success']==true){
  ...
  for($i=1;$i<=$release_respond;$i++){
    ...
    $teacherID = $feed['teacherID'];

    $get_question_info = curl_init("http://afsaccess1.njit.edu/~jkv5/CS490/studentmiddle_quiz.php");
    curl_setopt_array($get_question_info, array(
      CURLOPT_POST =>true,
      CURLOPT_POSTFIELDS =>json_encode(array('teacherID'=>$teacherID, 'questionID'=>$questionID,'questionType' =>  $questionType, 'role' => 'questioninfo')),
      CURLOPT_RETURNTRANSFER =>true,
    ));
  ...
  //  var_dump($feedback);
    ...
    if ($feedback['success'] == true){
      ...
      if ($questionType == "TF"){
        ...
        $middle .= "<tr><td style='padding-left:20px;padding-right:20px;'>Correct Solution: </td><td>".$myfeedback["correctAns"]."</td></tr>";
        $middle .= "<tr><td style='padding-left:20px;padding-right:20px;'>Answer:</td><td>".$answer."</td></tr>";
        for ($k=1;$k<=5;$k++){
          if(!empty($myfeedback['testcase'."$k"])&&!empty($myfeedback['answer'."$k"])){
            ...
            if($feed['feedback'."$k"]==$myfeedback['answer'."$k"]){
              $middle .= "<tr><td style='padding-left:20px;padding-right:20px;'>Your Output:</td><td><span style = 'color:green'>".$myfeedback['answer'."$k"]."&nbsp;&nbsp;<img src='../img/checkmark.png'/ width='20px' height='20px'></span></td></tr>";// fix this
            }else{ $middle .= "<tr><td style='padding-left:20px;padding-right:20px;'>Your Output:</td><td><span style = 'color:red'>".$feed['feedback'."$k"]."&nbsp;&nbsp;<img src='../img/Xmark.png'/ width='20px' height='20px'></span></td></tr>";}//fix this
          }
        }
      }
  ...
  $header = "<tr><td colspan='2'><span class='lead'>Quiz: $quiztitle</span></td></td></tr>";
